    ...
    for (unsigned i = 0; i <= n_runs; i++) {
        ...
        for (int j=0; j < numpanels[i]; j++){
            ...
            for (int m=0; m < N; m++){
                ...
                complex_t temp = ( res[j] * cont_space.evaluateShapeFunction(1, GaussQR.x(m))
                        +
                        //contribution of second shape fct.
                        res[(j+1)%numpanels[i]] * cont_space.evaluateShapeFunction(0, GaussQR.x(m))
                        -
                        //exact solution
                        fund_sol_dir(panels_coarse[j]->operator[](GaussQR.x(m)).x(), panels_coarse[j]->operator[](GaussQR.x(m)).y()));
        ...
        for (int j=0; j < numpanels[i]; j++){
            ...
            for (int m=0; m < N; m++){
                ...
                //contribution of first shape fct.
                complex_t temp = (
                        res_known[j] * cont_space.evaluateShapeFunction(1, GaussQR.x(m))
                        +
                //contribution of second shape fct.
                        res_known[(j+1)%numpanels[i]] * cont_space.evaluateShapeFunction(0, GaussQR.x(m))
                        -
                        //exact solution
                        fund_sol_dir(panels_coarse[j]->operator[](GaussQR.x(m)).x(), panels_coarse[j]->operator[](GaussQR.x(m)).y()));
                //squared norm multiplied by scaling factors
