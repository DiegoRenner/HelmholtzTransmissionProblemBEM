...
int main() {
    ...
    unsigned n_points_x = 200;
    unsigned n_points_y = 1;
    unsigned n_runs_N = 1;
    double numpanels[n_runs_N];
    numpanels[0] = 10;
    std::ofstream filename;
    filename.open("/home/diegorenner/Uni/Thesis/matlab_plots/SV_analysis_ext_debug.dat", std::ofstream::out | std::ofstream::trunc);
    filename.close();
        ...
        for (unsigned j = 0; j < n_points_x; j++) {
            for (unsigned k = 0; k < n_points_y; k++) {
                ...
                auto sv_eval = [&] (double k) {
                    k_o = (k*sqrt(n_o));
                    k_i = (k*sqrt(n_i));
                    return parametricbem2d::eval_sv_debug(mesh,order,k_o,k_i);
                };
                double der =  parametricbem2d::diffex(sv_eval,k_temp,0.000001,0.000001,0.0000000001);
                std::cout << "**********************" << std::endl;
                filename.open("/home/diegorenner/Uni/Thesis/matlab_plots/SV_analysis_ext_debug.dat", std::ios_base::app);
