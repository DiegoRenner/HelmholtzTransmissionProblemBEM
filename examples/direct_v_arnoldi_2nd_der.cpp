 ...
 * the second derivative of the singular values
 ...
 *
 * <tt>
 * /path/to/direct_v_arnoldi_2nd_der \<radius of circle\> \<number of SV derivatives to be computed\>
 * \<accurracy of arnoldi algorithm\>.
 * </tt>
 *
 * The script will generate four files:
 * file_vals_eig_\<number of SV derivatives to be computed\>_\<accurracy of arnoldi algorithm\>_2ndDer.dat,
 * file_vals_arpp_\<number of SV derivatives to be computed\>_\<accurracy of arnoldi algorithm\>_2ndDer.dat,
 * file_timings_\<number of SV derivatives to be computed\>_\<accurracy of arnoldi algorithm\>_2ndDer.dat,
 * file_iter_\<number of SV derivatives to be computed\>_\<accurracy of arnoldi algorithm\>_2ndDer.dat.
...
int main(int argc, char** argv) {
    ...
    for (unsigned j = 0; j < n_points_x; j++) {
        for (unsigned k = 0; k < n_points_y; k++) {
            ...
            res_arnoldi = arnoldi::sv_2nd_der(T,T_der,T_der2,count,acc);
            ...
            file_out << res_direct.block(0, 2, count, 1).transpose() << std::endl;
            file_out.close();
            // write singular values computed by Arnoldi algorithm to file
            file_out.open(file_vals_arpp, std::ios_base::app);
            file_out << k_temp.real() << " ";
            file_out << res_arnoldi.block(0, 2, count, 1).transpose() << std::endl;
            file_out.close();
