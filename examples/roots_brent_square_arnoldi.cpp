 ...
 *
 * <tt>
 *  /path/to/roots_brent_square_arnoldi \<half side length of square\> \<refraction inside\>
    ...
    for (unsigned j = 0; j < n_points_x; j++) {
        for (unsigned k = 0; k < n_points_y; k++) {
            ...
            auto sv_eval_der = [&] (double k_in) {
                ...
                so.gen_sol_op_1st_der(k_in, c_o, c_i, T, T_der);
                double res = arnoldi::sv_1st_der(T, T_der, count, acc)(m,1);
                auto end = high_resolution_clock::now();
                duration_ops += duration_cast<milliseconds>(end-start);
                return res;
            };
