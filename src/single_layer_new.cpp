  ...
  for (int i = 0; i < Q; ++i) {
    for (int j = 0; j < Q; ++j) {
      ...
      auto integrand1 = [&](double s, double t) {
        ...
        double s_st;
        if (fabs(s - t) > sqrt_epsilon) // Away from singularity for stable evaluation as
      ...
      for (unsigned int i = 0; i < N; ++i) {
        for (unsigned int j = 0; j < N; ++j) {
          ...
          integral += GaussQR.w(i) * GaussQR.w(j) *
                  sp_bessel::hankelH1(0,k*(pi[s]-pi_p[t]).norm())* F(t) * G(s);
        }
      }
      // Filling up the matrix entry
      interaction_matrix(i, j) = ii*integral/4.;
    }
  }
  return interaction_matrix;
}
