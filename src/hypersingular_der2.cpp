                ...
                for (int j = 0; j < Q; ++j) {
                    ...
                    auto integrand = [&](double s, double t) {
                        ...
                        if ( abs(k)*(pi[s]-pi_p[t]).norm() > epsilon ) {
                            result = (sp_bessel::hankelH1p(1,k*(pi[s]-pi_p[t]).norm())*(pi[s]-pi_p[t]).squaredNorm()*(F_arc(t) * G_arc(s) - k * k * F(t) * G(s) * normal.dot(normal_p))
                                      - sp_bessel::hankelH1(1,k*(pi[s]-pi_p[t]).norm())*(pi[s]-pi_p[t]).norm()*(4.*k*F(t)*G(s)*normal.dot(normal_p))
                                    + sp_bessel::hankelH1(0,k*(pi[s]-pi_p[t]).norm())*(2.0*F(t)*G(s)*normal.dot(normal_p)));
                        }
                        return result;
                    };
                    ...
                    auto integrand = [&](double s, double t) {
                        ...
                        if (swap){
                            if ( abs(k)*(pi[s]-pi_p.swapped_op(t)).norm() > epsilon ) {
                                result = (sp_bessel::hankelH1p(1,k*(pi[s]-pi_p.swapped_op(t)).norm())*(pi[s]-pi_p.swapped_op(t)).squaredNorm()*(F_arc(t) * G_arc(s) - k * k * F(t) * G(s) * normal.dot(normal_p))
                                          - sp_bessel::hankelH1(1,k*(pi[s]-pi_p.swapped_op(t)).norm())*(pi[s]-pi_p.swapped_op(t)).norm()*(4.*k*F(t)*G(s)*normal.dot(normal_p))
                                + sp_bessel::hankelH1(0,k*(pi[s]-pi_p.swapped_op(t)).norm())*(2.*F(t)*G(s)*normal.dot(normal_p)));
                            }
                        }else {
                            if ( abs(k)*(pi.swapped_op(s)-pi_p[t]).norm() > epsilon ) {
                                result = (sp_bessel::hankelH1p(1,k*(pi.swapped_op(s)-pi_p[t]).norm())*(pi.swapped_op(s)-pi_p[t]).squaredNorm()*(F_arc(t) * G_arc(s) - k * k * F(t) * G(s) * normal.dot(normal_p))
                                          - sp_bessel::hankelH1(1,k*(pi.swapped_op(s)-pi_p[t]).norm())*(pi.swapped_op(s)-pi_p[t]).norm()*(4.*k*F(t)*G(s)*normal.dot(normal_p))
                                + sp_bessel::hankelH1(0,k*(pi.swapped_op(s)-pi_p[t]).norm())*(2.*F(t)*G(s)*normal.dot(normal_p)));
                            }
                        }
                        return result;
                    };
