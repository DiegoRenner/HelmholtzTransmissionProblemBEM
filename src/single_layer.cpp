                    ...
                    auto integrand = [&](double s, double t) {
                        ...
                        if (swap) {
                            if ( (pi[s]-pi_p.swapped_op(t)).norm() > epsilon) {
                                result = complex_t(-y0(k*(pi[s]-pi_p.swapped_op(t)).norm()),
                                                   j0(k*(pi[s]-pi_p.swapped_op(t)).norm()));
                            };
                        } else {
                            if ( (pi.swapped_op(s)-pi_p[t]).norm() > epsilon) {
                                result = complex_t(-y0(k*(pi.swapped_op(s)-pi_p[t]).norm()),
                                                   j0(k*(pi.swapped_op(s)-pi_p[t]).norm()));
                            };
                        }
