            ...
            for (int i = 0; i < Qtest; ++i) {
                for (int j = 0; j < Qtrial; ++j) {
                    ...
                    auto integrand = [&](double s, double t) {
                        ...
                        result = (complex_bessel::H1p(1,k*sqrt(c)*(pi[s]-pi_p[t]).norm(),2)*(pi[s]-pi_p[t]).norm()*k*sqrt(c)
                                  + 2.*complex_bessel::H1p(1,k*sqrt(c)*(pi[s]-pi_p[t]).norm()))
                    ...
                    auto integrand = [&](double s, double t) {
                        ...
                        if (swap) {
                            if ( abs(k*sqrt(c))*(pi[s]-pi_p.swapped_op(t)).norm() > epsilon ) { // Away from singularity
                                result = (complex_bessel::H1p(1,k*sqrt(c)*(pi[s]-pi_p.swapped_op(t)).norm(),2)*(pi[s]-pi_p.swapped_op(t)).norm()*k*sqrt(c)
                                         +2.*complex_bessel::H1p(1,k*sqrt(c)*(pi[s]-pi_p.swapped_op(t)).norm()))
                                         *(pi[s]-pi_p.swapped_op(t)).dot(normal);
                            }
                        } else {
                            if ( abs(k*sqrt(c))*(pi.swapped_op(s)-pi_p[t]).norm() > epsilon ) { // Away from singularity
                                result = (complex_bessel::H1p(1,k*sqrt(c)*(pi.swapped_op(s)-pi_p[t]).norm(),2)*(pi.swapped_op(s)-pi_p[t]).norm()*k*sqrt(c)
                                             +2.*complex_bessel::H1p(1,k*sqrt(c)*(pi.swapped_op(s)-pi_p[t]).norm()))
