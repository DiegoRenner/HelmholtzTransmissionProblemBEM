...
inline void GalerkinMatrixBuilder::compute_coinciding(const AbstractParametrizedCurve &p) throw() {
    ...
    p.Derivative_01(m_tc, m_tangent_p[0], m_tangent_p_norm[0]);
    p.Derivative_01(m_sc, m_tangent[0], m_tangent_norm[0]);
    m_v[0] = p[m_sc] - p[m_tc];
...
inline void GalerkinMatrixBuilder::compute_adjacent(const AbstractParametrizedCurve &pi, const AbstractParametrizedCurve &pi_p, bool swap) throw() {
    ...
    if (swap) {
        pi_p.Derivative_01_swapped(m_ta, m_tangent_p[0], m_tangent_p_norm[0], true);
        pi_p.Derivative_01_swapped(m_sa, m_tangent_p[1], m_tangent_p_norm[1], true);
        pi.Derivative_01(m_sa, m_tangent[0], m_tangent_norm[0]);
        pi.Derivative_01(m_ta, m_tangent[1], m_tangent_norm[1]);
    } else {
        pi_p.Derivative_01(m_ta, m_tangent_p[0], m_tangent_p_norm[0]);
        pi_p.Derivative_01(m_sa, m_tangent_p[1], m_tangent_p_norm[1]);
        pi.Derivative_01_swapped(m_sa, m_tangent[0], m_tangent_norm[0], true);
        pi.Derivative_01_swapped(m_ta, m_tangent[1], m_tangent_norm[1], true);
    }
...
inline void GalerkinMatrixBuilder::compute_general(const AbstractParametrizedCurve &pi, const AbstractParametrizedCurve &pi_p) throw() {
    ...
    pi_p.Derivative_01(m_tg, m_tangent_p_s, m_tangent_p_norm_s);
    pi.Derivative_01(m_sg, m_tangent_s, m_tangent_norm_s);
    m_v_s = pi[m_sg] - pi_p[m_tg];
    m_v_norm_s = m_v_s.cwiseAbs();
    auto tic = chrono::high_resolution_clock::now();
    if (k_real_positive) {
...
void GalerkinMatrixBuilder::double_layer_adjacent(bool swap, int der, bool transp) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        const Eigen::ArrayXXd &fg = swap ? ((transp ? K == 0 : K == 1) ? m_double_layer_adjacent_fg_swap_t
                                                                       : m_double_layer_adjacent_fg_swap)
                                         : ((transp ? K == 0 : K == 1) ? m_double_layer_adjacent_fg_t
                                                                       : m_double_layer_adjacent_fg);
        for (j = 0; j < Qtrial; ++j) {
            for (i = 0; i < Qtest; ++i) {
...
void GalerkinMatrixBuilder::double_layer_general(int der, bool transp) throw() {
    ...
    const Eigen::ArrayXXd &fg = transp ? m_double_layer_general_fg_t : m_double_layer_general_fg;
    if (!transp) {
        vdotn = m_wg * (m_v_s.real() * m_tangent_p_s.imag() - m_v_s.imag() * m_tangent_p_s.real()) * m_tangent_norm_s;
        auto mask = ksqrtca * m_v_norm_s > epsilon;
        masked1_s = mask.select(ksqrtc * m_h1_s / m_v_norm_s, (m_v_norm_s > epsilon).select(M_2_PI / (m_v_norm_s * m_v_norm_s), m_zero_s));
        if (der > 0)
            masked2_s = mask.select(m_h0_s, m_zero_s);
        if (der > 1)
            masked3_s = masked2_s - mask.select(ksqrtc * m_h1_s * m_v_norm_s, m_zero_s);
    } else vdotn = m_wg * (m_v_s.imag() * m_tangent_s.real() - m_v_s.real() * m_tangent_s.imag()) * m_tangent_p_norm_s;
    for (j = 0; j < Qtrial; ++j) {
        for (i = 0; i < Qtest; ++i) {
            cf = vdotn * fg.block(i * N, j * N, N, N);
        ...
        for (i = 0; i < Qtrial; ++i) {
            ...
            if (der > 0)
                hypersingular_der_interaction_matrix(i, j) =
                    mask.select(h1_vnorm * cf + 2.0 * h0 * ksqrtc * temp, m_zero_s).sum();
            if (der > 1)
                hypersingular_der2_interaction_matrix(i, j) = mask.select(
                    (h0 * v_norm * v_norm - h1_vnorm / ksqrtc) * cf - 2.0 * temp * (2.0 * ksqrtc * h1_vnorm - h0), m_zero_s).sum();
        }
    }
    if (der > 0)
        hypersingular_der_interaction_matrix *= -sqrtc;
...
void GalerkinMatrixBuilder::single_layer_general(int der) throw() {
    ...
    Eigen::ArrayXXd cf, ttp_norm = m_tangent_norm_s * m_tangent_p_norm_s * m_wg;
    auto mask = ksqrtca * v_norm > epsilon;
    masked1_s = mask.select(h0, (v_norm > epsilon).select(-M_2_PI * v_norm.log(), m_zero_s));
    if (der > 0)
...
void GalerkinMatrixBuilder::all_coinciding(int der) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        const Eigen::ArrayXXd &fg = K == 0 ? m_double_layer_coinciding_fg : m_double_layer_coinciding_fg_t;
        for (j = 0; j < Q; ++j) {
            for (i = 0; i < Q; ++i) {
                cfr = vdotn * fg.block(i * N, j * N, N, N);
...
void GalerkinMatrixBuilder::all_adjacent(bool swap, int der) throw() {
    ...
    for (K = 0; K < 2; ++K) {
        ...
        const Eigen::ArrayXXd &fg = swap ? (K == 1 ? m_double_layer_adjacent_fg_swap_t : m_double_layer_adjacent_fg_swap)
                                         : (K == 1 ? m_double_layer_adjacent_fg_t : m_double_layer_adjacent_fg);
        for (j = 0; j < Q; ++j) {
            for (i = 0; i < Q; ++i) {
                cfr = vdotn * fg.block(i * N, j * N, N, N);
...
void GalerkinMatrixBuilder::all_general(int der) throw() {
    ...
    Eigen::ArrayXXd ttp_norm = m_tangent_norm_s * m_tangent_p_norm_s * m_wg;
    Eigen::ArrayXXd tdottp = (m_tangent_s.imag() * m_tangent_p_s.imag() + m_tangent_s.real() * m_tangent_p_s.real()) * m_wg;
    Eigen::ArrayXXd v_norm2 = m_v_norm_s * m_v_norm_s;
    auto mask = ksqrtca * m_v_norm_s > epsilon;
    auto mask2 = m_v_norm_s > epsilon;
    masked1_s = mask.select(ksqrtc * m_h1_s / m_v_norm_s, mask2.select(M_2_PI / v_norm2, m_zero_s));
    Eigen::ArrayXXcd masked1_hg = mask.select(m_h0_s, mask2.select(-M_2_PI * m_v_norm_s.log(), m_zero_s));
    Eigen::ArrayXXd vdotn = m_wg * (m_v_s.real() * m_tangent_p_s.imag() - m_v_s.imag() * m_tangent_p_s.real()) * m_tangent_norm_s;
    Eigen::ArrayXXcd masked2_g, masked3_g, cf;
    ...
    if (der > 1) {
        ...
        masked3_g = masked2_g / ksqrtc - masked2_s * v_norm2;
    }
    for (j = 0; j < Q; ++j) {
        for (i = 0; i < Q; ++i) {
            ...
            if (der > 0) {
                ...
                hypersingular_der_interaction_matrix(i, j) = (masked2_g * cf + (2.0 * ksqrtc) * masked2_s * temp).sum();
            }
            if (der > 1) {
                double_layer_der2_interaction_matrix(i, j) = (cfr1 * masked3_s).sum();
                single_layer_der2_interaction_matrix(i, j) = (cfr2 * masked3_g).sum();
                hypersingular_der2_interaction_matrix(i, j) = -(masked3_g * cf + 2.0 * temp * (masked2_s - 2.0 * masked3_s)).sum();
            }
        }
    }
    if (der > 0) {
