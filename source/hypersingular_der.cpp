...
namespace hypersingular_helmholtz_der {
    ...
    void ComputeIntegralCoinciding(Eigen::MatrixXcd &interaction_matrix,
        ...
        complex_t ksqrtc_i = k * sqrtc_i, ksqrtc_o = k * sqrtc_o, ksqrtc2_i = k * k * c_i, ksqrtc2_o = k * k * c_o;
        ...
        auto integrand = [&](int i, int j, int m) {
            const double &s = ws.s(m), &t = ws.t(m), &n_dot_np = ws.n_dot_np(m);
            ...
            complex_t r_o1 = sqrtc_o * ws.result_o(m), r_o2 = sqrtc_o * ws.result_o(m, true);
            complex_t r_i1 = sqrtc_i * ws.result_i(m), r_i2 = sqrtc_i * ws.result_i(m, true);
            return (r_o1 - r_i1) * F_arc * G_arc + (r_o2 - r_i2 - r_o1 * ksqrtc2_o + r_i1 * ksqrtc2_i) * F * G * n_dot_np;
        };
        for (int i = 0; i < Q; ++i) {
            for (int j = 0; j < Q; ++j) {
    ...
    void ComputeIntegralAdjacent(Eigen::MatrixXcd &interaction_matrix,
        ...
        std::for_each (std::execution::par_unseq, ws.ind().cbegin(), ws.ind().cbegin() + N2, [&](auto &i) {
            ...
            result_i = result_o = result_ir = result_or = czero;
            double &t = ws.t(i), &s = ws.s(i), &w = ws.w(i), &t_norm = ws.t_norm(i), &tp_norm = ws.tp_norm(i), &n_dot_np = ws.n_dot_np(i);
            s = GaussQR.x(I) * GaussQR.x(J);
            t = GaussQR.x(I);
            w = GaussQR.x(I) * GaussQR.w(I) * GaussQR.w(J);
            // Finding the tangent of pi_p to get its normal
        ...
        auto integrand = [&](int i, int j, int m) {
            ...
            complex_t r_o1 = sqrtc_o * ws.result_o(m), r_o2 = sqrtc_o * ws.result_o(m, true);
            complex_t r_i1 = sqrtc_i * ws.result_i(m), r_i2 = sqrtc_i * ws.result_i(m, true);
            return (r_o1 - r_i1) * F_arc * G_arc + (r_o2 - r_i2 - ksqrtc2_o * r_o1 + ksqrtc2_i * r_i1) * F * G * n_dot_np;
        };
        ...
        for (int i = 0; i < Q; ++i) {
            for (int j = 0; j < Q; ++j) {
                ...
                -ii * 0.25 * std::accumulate(ws.IJ().cbegin(), ws.IJ().cbegin() + N2, czero, [&](const auto &sum, const auto &ij) {
                    ...
                    complex_t r_o1 = sqrtc_o * ws.result_o(m), r_o2 = sqrtc_o * ws.result_o(m, true);
                    complex_t r_i1 = sqrtc_i * ws.result_i(m), r_i2 = sqrtc_i * ws.result_i(m, true);
                    return sum + ws.w(m) * ((r_o1 - r_i1) * F_arc * G_arc + (r_o2 - r_i2 - r_o1 * ksqrtc2_o + r_i1 * ksqrtc2_i) * F * G * n_dot_np);
                });
            }
        }
    }
