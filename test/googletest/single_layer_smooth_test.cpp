...
double sqrt_epsilon = std::sqrt(std::numeric_limits<double>::epsilon());
    ...
    Eigen::VectorXcd V_smooth = parametricbem2d::single_layer_smooth_helmholtz::GalerkinMatrix(mesh, discont_space, order, c_o).block(0, 0, 1,
                                                                                                           numpanels).transpose();
    Eigen::VectorXcd V_correction = parametricbem2d::single_layer_correction::GalerkinMatrix(mesh, discont_space, order,k).block(0, 0, 1,
                                                                                                              numpanels).transpose();
    Eigen::VectorXcd V = V_smooth + V_correction;
    ...
    int i = 0;
    std::string path =
            "/home/diegorenner/Uni/Thesis/HelmholtzBEM/raw_data/single_layer_o_" + std::to_string(numpanels) + ".dat";
    ...
    TEST(SingleLayerTest, disjoint_fair) {
        ...
        std::cout << "***********************************" << std::endl;
        std::cout << (V.segment(2, numpanels - 3) - V_expected.segment(2, numpanels - 3)).lpNorm<2>() / (numpanels - 3)
                  << std::endl;
        ASSERT_TRUE((V.segment(2, numpanels - 3) - V_expected.segment(2, numpanels - 3)).lpNorm<2>() / (numpanels - 3) <
                    sqrt_epsilon);
        fp_data.close();
    }
